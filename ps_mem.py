...
def cmd_with_count(cmd, count):
	if count > 1: return "%s (%u)" % (cmd, count)
	else: return cmd
...
def get_memory_usage(
	...
	for pid in os.listdir(proc.path('')):
		...
		# Some filters
		if only_self and pid != our_pid: continue
		if pid == our_pid and not include_self: continue
		if pids_to_show is not None and pid not in pids_to_show: continue
...
def print_memory_usage(
	sorted_cmds, shareds, count,
	total, swaps, total_swap, show_swap
):
...
def main():
	...
	if watch is not None:
		try:
			sorted_cmds = True
			while sorted_cmds:
				sorted_cmds, shareds, count, total, swaps, total_swap = get_memory_usage(pids_to_show, split_args, discriminate_by_pid)
				if only_total and show_swap and have_swap_pss:
					...
					print_memory_usage(sorted_cmds, shareds, count, total, swaps, total_swap, show_swap)
